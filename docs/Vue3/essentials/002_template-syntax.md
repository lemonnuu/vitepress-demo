...
如果你对虚拟 DOM 的概念比较熟悉，并且偏向于 JavaScript 的原始力量，也<span style="background: rgba(122, 205, 166, 0.5)">可以结合可选的 JSX 支持直接手写渲染函数而不采用模板。但请注意，这将不会享受到和模板同等级别的编译时优化。</span>
...
> 接下来，都将在示例中使用简写语法，因为这是在 Vue 开发者之间最常见的用法。
...
:::danger 布尔型 attribute 绑定存在差异
当 `isButtonDisabled` 为除了空字符串的其他假值时, "disabled" 才会被忽略。
:::
...
<span style="background: rgba(122, 205, 166, 0.5)">这些表达式都会被作为 JavaScript ，以组件为作用域解析执行。</span>
...
:::tip 最佳实践
`app.config.globalProperties` 可以代替 Vue2 的 `Vue.prototype.xxx`
:::
...
<span style="background: rgba(122, 205, 166, 0.5)">动态参数表达式因为某些字符的缘故有一些语法限制，比如空格和引号</span>，在 HTML attribute 名称中都是不合法的。例如下面的示例：
...
如果需要传入一个复杂的动态参数，推荐<span style="background: rgba(254, 43, 80, 0.5)">使用计算属性替换复杂的表达式</span>，也是 Vue 最基础的概念之一。
