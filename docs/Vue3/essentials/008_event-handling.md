...
<span style="background: rgba(242, 191, 69, 0.5)">方法事件处理器会自动接收原生 DOM 事件并触发执行</span>。在上面的例子中，我们能够通过被触发事件的 `event.target.tagName` 访问到该 DOM 元素。
...
<span style="background: rgba(242, 191, 69, 0.5)">有时我们需要在内联事件处理器中访问原生 DOM 事件。你可以向该处理器方法传入一个特殊的 `$event` 变量，或者使用内联箭头函数：</span>
