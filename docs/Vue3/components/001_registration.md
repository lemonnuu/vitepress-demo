...
1. <span style="background: rgba(122, 205, 166, 0.5)">全局注册使构建系统无法移除未使用的组件 (也叫“tree-shaking”)。如果你全局注册了一个组件，却一次都没有使用，它仍然会出现在最终的构建产物中。</span>

2. 全局注册在大型项目中使项目的依赖关系变得不那么明确。在父组件中使用子组件时，很难定位子组件的实现。这可能<span style="background: rgba(122, 205, 166, 0.5)">会影响未来长期的可维护性</span>，类似于使用过多的全局变量。
...
请注意：<span style="background: rgba(242, 191, 69, 0.5)">**局部注册组件在后代组件中并<i>不</i>可用**</span>。在这个例子中，`ComponentA` 注册后仅在当前组件可用，而在任何的子组件或后代组件中都不可用。
